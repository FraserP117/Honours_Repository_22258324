...
class Agent():
    ...
    def set_parameters(self, argv):
        ...
        default_parameters = {'run_id':"_rX", 'device':'cpu',
              'env':'CartPole-v1', 'n_episodes':3100, 
    ...
    #     pred_batch_t0t1_mean, pred_batch_t0t1_var = self.transition_net(X)

    #     # Determine the prediction error wrt time t0-t1:
    #     pred_error_batch_t0t1 = torch.mean(F.mse_loss(
    #             pred_batch_t0t1_mean, obs_batch_t1, reduction='none'), dim=1).unsqueeze(1)
    ...
    def get_mini_batches(self):
        ...
        X = torch.cat((obs_batch_t0, action_batch_t0.float()), dim=1) # move alll this to get mini batches
        pred_batch_mean_t0t1_theta, pred_batch_var_t0t1_theta = self.generative_transition_net(X) # move alll this to get mini batches
        pred_batch_mean_t0t1_phi, pred_batch_var_t0t1_phi = self.variational_transition_net(action_batch_t0.float()) # move alll this to get mini batches
        ...
        # # Determine the prediction error wrt time t0-t1:
        # pred_error_batch_t0t1 = torch.mean(F.mse_loss(
        #         pred_batch_mean_t0t1_theta, obs_batch_t1, reduction='none'), dim=1).unsqueeze(1) # COULD TRY SAMPLES INSTEAD 

        D_KL_state_pred_batch = self.kl_divergence_diag_cov_gaussian(
             pred_batch_mean_t0t1_theta, pred_batch_var_t0t1_theta,
             pred_batch_mean_t0t1_phi, pred_batch_var_t0t1_phi
        )
    ...
    def compute_value_net_loss(
        ...
        terminated_batch_t2, truncated_batch_t2, VFE_state_pred_batch
    ):

    # def compute_value_net_loss( # LAST FUNCTIONAL IMP
    #     self, obs_batch_t1, obs_batch_t2, action_batch_t1, reward_batch_t1,
    ...
    # ):
        ...
        with torch.no_grad():
            ...
            # # calculate KL divergence for the state inference:
            # # pred_batch_mean_t0t1_theta, pred_batch_var_t0t1_theta,
            # # pred_batch_mean_t0t1_phi, pred_batch_var_t0t1_phi

            # # Sample the predicted Q_phi and P_theta
            # Q_phi_samples = self.variational_transition_net.reparameterize(pred_batch_mean_t0t1_phi, pred_batch_var_t0t1_phi)
            # P_theta_samples = self.generative_transition_net.reparameterize(pred_batch_mean_t0t1_theta, pred_batch_var_t0t1_theta)

            # # Weigh them according to the action distribution:
            # energy_batch = -(Q_phi_samples * torch.log(P_theta_samples)).sum(-1).view(self.batch_size, 1) 
            
            # # Determine the entropy of the action distribution
            # entropy_batch = -(P_theta_samples * torch.log(P_theta_samples)).sum(-1).view(self.batch_size, 1)

            # VFE_state_pred_batch = (energy_batch - entropy_batch)
            ...
            # Determine the batch of bootstrapped estimates of the EFEs:
            # EFE_estimate_batch = -reward_batch_t1 + pred_error_batch_t0t1 + self.Beta * weighted_EFE_value_targets # LAST FUNCTIONAL IMP
            EFE_estimate_batch = -reward_batch_t1 + VFE_state_pred_batch + self.Beta * weighted_EFE_value_targets
    ...
    # def compute_VFE(self, obs_batch_t1, pred_error_batch_t0t1): # LAST FUNCTIONAL IMP
    ...
    # ):
        ...
        # # Weigh them according to the action distribution:
        # energy_state_pred_batch = -(Q_phi_samples * torch.log(P_theta_samples)).sum(-1).view(self.batch_size, 1) # IS NAN

        # # Determine the entropy of the action distribution
        # entropy_state_pred_batch = -(P_theta_samples * torch.log(P_theta_samples)).sum(-1).view(self.batch_size, 1) # IS NAN

        # VFE_state_pred_batch = (energy_state_pred_batch - entropy_state_pred_batch)
        #########################################################################################################################################################
        ...
        # print(f"\n\nVFE_state_pred_batch: {VFE_state_pred_batch}")
        # print(f"(energy_policy_pred_batch - entropy_policy_pred_batch): {(energy_policy_pred_batch - entropy_policy_pred_batch)}\n\n")
        
        # Determine the VFE, then take the mean over all batch samples:
        # VFE_batch = pred_error_batch_t0t1 + (energy_policy_pred_batch - entropy_policy_pred_batch) # LAST FUNCTIONAL IMP
        VFE_batch = VFE_state_pred_batch + (energy_policy_pred_batch - entropy_policy_pred_batch)


        # print(f"\nenergy_state_pred_batch: {energy_state_pred_batch}")
        # print(f"entropy_state_pred_batch: {entropy_state_pred_batch}")
        # print(f"energy_policy_pred_batch: {energy_policy_pred_batch}")
        # print(f"entropy_policy_pred_batch: {entropy_policy_pred_batch}\n")

        # VFE_policy_pred_batch = (energy_policy_pred_batch - entropy_policy_pred_batch)

        # # Determine the VFE, then take the mean over all batch samples:
        # VFE_batch = VFE_state_pred_batch + VFE_policy_pred_batch

        VFE = torch.mean(VFE_batch)
                
        return VFE

    def compute_VFE_pomdp(self, vae_loss, state_batch_t1, pred_error_batch_t0t1):
    ...
    def learn(self):
        ...
        # Retrieve transition data in mini batches: LAST FUNCTIONAL IMPLEMENTATION
        # (
        #     obs_batch_t0, obs_batch_t1, obs_batch_t2, action_batch_t0,
        #     action_batch_t1, reward_batch_t1, terminated_batch_t2, truncated_batch_t2,
        #     pred_error_batch_t0t1
        ...
        # # Compute the value network loss: LAST FUNCTIONAL IMPLEMENTATION
        # efe_value_net_loss = self.compute_value_net_loss(
        #     obs_batch_t1, obs_batch_t2,
        #     action_batch_t1, reward_batch_t1,
        #     terminated_batch_t2, truncated_batch_t2, pred_error_batch_t0t1
        ...
        efe_value_net_loss = self.compute_value_net_loss(
            obs_batch_t1, obs_batch_t2,
            action_batch_t1, reward_batch_t1,
            terminated_batch_t2, truncated_batch_t2, D_KL_state_pred_batch
        )
        ...
        # Compute the variational free energy: LAST FUNCTIONAL IMPLEMENTATION
        # VFE = self.compute_VFE(obs_batch_t1, pred_error_batch_t0t1)

        VFE = self.compute_VFE(obs_batch_t1, D_KL_state_pred_batch)
