...
class Agent_DQN():
    ...
    def __init__(self, argv):
        ...
        self.memory = ReplayMemory_DQN(self.memory_capacity, self.obs_shape, device=self.device)
    ...
    def set_parameters(self, argv):
        ...
        default_parameters = {'run_id':"_rX", 'device':'cuda',
              'env':'CartPole-v1', 'n_episodes':500, 
              'n_hidden':64, 'lr':1e-3,
    ...
    def learn(self):
        ...
        all_obs_batch, action_batch, reward_batch, terminated_batch, truncated_batch = self.memory.sample(
                self.obs_indices, self.action_indices, self.reward_indices, self.terminated_indices, self.truncated_indices, self.max_n_indices, self.batch_size)
        ...
        target_batch = reward_batch + (1-(terminated_batch | truncated_batch)) * self.gamma * target_out.max(1)[0].view(self.batch_size, 1)
    ...
    def train(self):
        ...
        filename = f"Deep_RL_MDP_Cart_Pole_v1"
